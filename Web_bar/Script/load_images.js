...
httpr.open("GET","../../Script/get_data.php?getter="+getter_type,true);
...
httpr.onreadystatechange=function(){
    if(httpr.readyState == 4 && httpr.status == 200){
        ...
        for(var i = 0; i < (numb/3); i++){
            var newrow = document.createElement("div");
            newrow.classList.add("row");
            newrow.classList.add("my-5");
            pics.appendChild(newrow);
            var maxpicincol = 3;
            if(numb - count < 3){
                maxpicincol = numb-count;
            }
            for(var j = 0; j < maxpicincol; j++){
                
                var newdiv1 = document.createElement("div");
                newdiv1.classList.add("col-4");
                var newdiv2 = document.createElement("div");
                newdiv2.classList.add("card");
                newdiv2.style.width = "18rem";
                var newimg = document.createElement("img");
                newimg.src = "../../Sources/" + data[count].path;
